...
│                         `vibe install gstack`
│
总计: 安装默认技能包后 40+ 个技能，持续增长中
（内置数量以 `ls core/skills | wc -l` 为准）
```
...
| `adversarial-panel` | 拉取最新后冻结评审面，五路独立对抗 + 先驳回核验 | 拉取最新 / 多路独立对抗复审 |
| `adversarial-arbitration` | 对抗合成器 REQUEST CHANGES 按 D/T/J 仲裁（会诊不是门禁） | 合成器 REQUEST CHANGES / 真问题还是看法 |
