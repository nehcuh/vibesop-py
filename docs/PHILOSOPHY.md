...
实验之后的人话版（准不准比「有没有匹配模块」更致命）：[给 AI 找对说明书](skill-routing-explained.md)。

### 2. 匹配 > 猜测

人类的直觉和记忆是有限的：
- 你记不住所有技能的命令（内置 + 技能包动辄几十个）
- 你无法准确判断每个场景该用什么
- 猜测会导致错误的工具选择
...
真正强大的系统不是"你输入一次，它输出一次"。而是**你设定一次，它持续运行**。

- ❌ **错误**: "每次任务都由人手动触发" → 瓶颈在人
- ✅ **正确**: "设定好的任务自动持续执行" → 瓶颈在系统
...
>
> — Hanako (@hanakoxbt), 2026

**体现**: vibe loop — 设定一次，持续运行，人在回路外（95% 的枯燥工作）。
...
**认知负荷**: 用户需要记忆大量技能和命令（18 个内置 + 安装后的技能包合计数十个）
...
**当前**: 18 个内置技能 + 技能包生态（superpowers/omx/gstack 等，安装后合计 40+）
**目标**: 100+ 技能
**方法**: 生态扩展
...
**成为 AI 辅助开发的标准技能操作系统（SkillOS）+ 自主任务引擎**
...
> **L0 不包含**：代码修改、合并 PR、架构决策——这些仍是 L3 的职责，需要人参与。
> **L2 不包含**：文件读写、代码修改、API 调用——这些是 L3 的职责。
> 简单任务由 VibeSOP 端到端完成（L1→L2），复杂任务由 AI Agent 接手（L3），**重复任务由 L0 自主接管**。
...
> **定位边界**: VibeSOP 是 SkillOS + 轻量引导执行层，管理技能的**全生命周期**：
> **发现 → 安装 → 路由 → 编排 → [L2引导执行 | L3 Agent接管] → 评估 → 保留/淘汰**
>
> 对于简单的单步骤任务，VibeSOP 提供端到端的"路由→注入→执行"体验（L1+L2）。
> 对于复杂的多步骤编排，VibeSOP 负责"用什么技能、以什么顺序"，具体执行由 AI Agent 完成（L3）。
> 两者渐进增强，确保平台无关性。
>
...
| loop-manager | 系统工作流（v8.0） | 自主循环任务的全生命周期管理：创建、调度、监控、归档 |
| loop-ci-watcher | 系统工作流（v8.0） | CI 状态自动监控与失败分析循环 |
| loop-daily-digest | 系统工作流（v8.0） | 每日项目状态汇总报告循环 |
