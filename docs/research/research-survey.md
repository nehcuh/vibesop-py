...
- [x] **T+21 复检结果**（2026-09-21 补跑，窗上界 09-14）：回声 **2.57 对/天**（18/7），≈ T+7 3.57 与 T+14 剔除 9-06 后的 2.5。H9 更新为「稳态日可维持 T+7 量级，T+14 反弹系单日事件」；不回滚。混杂：该窗 hook 入流降至 31.7/天。产物 `gate43-t21-echo-measure.md` + `scripts/measure_gate43_echo.py`。
...
| F9 | 学习环的敌人是自己的观测污染：-75.2% 后反弹 +264%；T+21 回到 2.57 对/天（单日事件，不回滚）；promote 成功仅 1 例（人审路径，自动 0 例） | H9，gate41-43（§6） | T+21 窗 hook 流量下降，不得把安静写成疗效 |
...
| 3 | R8 盲评结算（F1/F3）；「大模型编排 + 小模型执行」整机流水线单独立项并预注册（F2；方向提案写明 E4 未绿不做整机） | F1/F3（前半）、F2（后半） | 人评仍在；2026-09-21 查双臂产物仍在，**盲评未结算**（编排者已知标签，不能当盲评人；见 `ab-quant-r8-blind-status.md`）。E4 仍关 |
| 4 | gate43 T+21 复检（2026-09-14）裁决：回滚 or 归因确认——反弹归因未终裁前不得宣称治理成功 | F9 | **已跑**（09-21）：T+21 post 2.57 对/天，不回滚；见 `gate43-t21-echo-measure.md` |
