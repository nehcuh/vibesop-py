...
**Important**: VibeSOP generates a **configuration fragment** that should be merged with Kimi Code CLI's auto-generated default configuration. Do not replace the entire config file.
...
### 2. Initialize Kimi Code CLI (First Time Only)

```bash
# Run Kimi Code CLI - it will auto-generate default config
kimi

# In the Kimi Code CLI prompt, authenticate:
/login
```
...
cp ~/.kimi-code/config.toml ~/.kimi-code/config.toml.backup

# Append VibeSOP configuration to existing config
cat .vibe/dist/kimi-cli/config.toml >> ~/.kimi-code/config.toml
```

Or manually copy the `[vibesop.*]` sections from the generated config to your `~/.kimi-code/config.toml`.
...
# Deploy new skills
cp -r .vibe/dist/kimi-cli/skills ~/.kimi-code/
```
...
cp -r .vibe/dist/kimi-cli/skills/* .kimi-code/skills/
```

When using project-level skills, Kimi Code CLI automatically discovers them when you run `kimi` inside the project directory.
...
This opens a browser for authentication. Kimi Code CLI will automatically add the `api_key` field to your configuration.
...
Edit `~/.kimi-code/config.toml` and add the api_key field:
...
### What Kimi Code CLI Auto-Generates
...
After merging, your `~/.kimi-code/config.toml` should contain both Kimi Code CLI's default configuration and VibeSOP's additions:

```toml
# Kimi Code CLI Default Configuration
...
**Do not** replace the entire `~/.kimi-code/config.toml` with VibeSOP's generated file. Merge only the `[vibesop.*]` sections.
...
**Check**:
1. Skills directory exists: `~/.kimi-code/skills/` (or `.kimi-code/skills/` for project-level)
...
1. Remove existing config: `rm ~/.kimi-code/config.toml`
...
| Kimi Code CLI main config | `~/.kimi-code/config.toml` |
| Deployed skills | `~/.kimi-code/skills/` |
| Kimi Code CLI logs | `~/.kimi-code/logs/kimi.log` |

## Adapter Implementation

The Kimi Code CLI adapter is implemented in:
...
- [Kimi Code CLI Official Docs](https://moonshotai.github.io/kimi-code/)
- [Architecture](architecture/ARCHITECTURE.md) - System architecture

## Sources

- [Kimi Code CLI 命令参考](https://moonshotai.github.io/kimi-code/zh/reference/kimi-command.html)
- [Kimi Code CLI 配置文件文档](https://moonshotai.github.io/kimi-code/zh/configuration/config-files.html)
- [使用Kimi Code CLI 调用Kimi 大模型](https://platform.kimi.com/docs/guide/kimi-cli-support)
