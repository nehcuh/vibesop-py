...
>
> 全文不需要前置知识。结论来自 2026-08 至 09 的对照（R1–R8）：小样本、单次运行，**不是统计证明**。读过 Anthropic / Warp 那篇的人可以对上号；第一次听说 VibeSOP 的人，文末有最短安装路径。实现以 [路由系统设计](architecture/routing-system.md) 为准。
...
下面这张图是**本文的分析框架**，用来标约束从软到硬怎么升，**不是** Warp 原文里的结构图：
...
Warp 的解法是**两个** skill：Base（基础技能）承载工作指令；Improver（改进技能）当定时观察者，对比 Agent 当时的建议和人的回应，对 Base 提出尽量小的修改，走 GitHub 的 PR。定时侧是 GitHub Action / Oz 观察者。

```mermaid
flowchart TD
    A["Base Skill 基础技能"] --> B["Agent 带着它执行代码评审"]
    B --> C["人类反馈：点赞 / 点踩 + 文字纠正"]
    C --> D["Improver Skill<br/>GitHub Action / Oz 定时观察者"]
    D --> E["对 Base 提尽量小的修改"]
    E --> F["Git PR：可审查、可批准、可拒绝"]
    F --> A
...
路由不是一次「算个相似度」的调用。下面是**简化视图**，实现、阈值、分支条件以 [architecture/routing-system.md](architecture/routing-system.md) 为准：

```mermaid
flowchart TD
    Q["任务出现"] --> E{"1 显式点名？"}
    E -- "是" --> I["注入 SKILL.md<br/>至多约 3000 字符，超长截断"]
    E -- "否" --> L{"查询偏短？"}
    L -- "短" --> S["2 场景 / 语义索引"]
    L -- "长" --> T["3 LLM 分诊"]
    S --> M["4 匹配器并行聚合"]
    T --> M
    M -- "够像" --> I
    M -- "低于阈值" --> N["终端：no-match<br/>不注入技能正文"]
    M -- "兜底" --> F["终端：fallback-llm"]
    I --> R["命令 agent 再读原文件"]
    style N fill:#fce8e6,stroke:#ea4335
    style I fill:#e6f4ea,stroke:#34a853
```

四段是：显式点名 → 短查询走场景 / 语义索引 → 长查询走 LLM 分诊 → 匹配器并行聚合。终端态有两个：**no-match** 和 **fallback-llm**。不是「精确匹配失败就语义，语义失败就结束」。
...
不要把测试覆盖率、评测集分数、徽章和建议门禁画成同一张闸。能挡合并的，只有下面这一行硬门。

| 机制 | 口径 | 类型 | 实际挡住什么 |
|---|---|---|---|
| hermetic 路由指纹 | 34 条金标；当前 30/34 = **88%**；embedding 与 AI triage **关闭**；测 keyword / explicit 不回归。改 `SKILL.md` 内容哈希变了必须 `--update-baseline`，STALE **挡合并** | **硬门** | 改一句话导致 keyword/explicit 路由静默翻盘 |
| 影子验证 | `vibe skill promote` 之后贴 PASS / WARN 徽章；测的是 trigger 召回，不是内容质量；**永不阻断激活** | **软口径** | 给人看「这次召回如何」，不是上岗许可 |
| R1–R8 对照 | 小样本、单次运行；见第 6、7 节 | **实验观察** | 不能当平均提升，也不能当 CI 契约 |

人能审出「这句话写得对不对」，审不出「这句话会不会改掉 hermetic 那 34 条的 top-1」。所以内容变了，基线必须重验。它锁的是 keyword / explicit 行为，不是「语义层已经过关」——语义层在这道门里是关掉的。

仓库另有 pytest 分支覆盖 `fail_under=73`。它测的是 **Python 代码**，不是技能效果，所以不进这张表。

## 6. 一次失败观察：27B 没有去读文件

R6 不是路由管线的起源，也不是「file-based skill 必须先有入口」的定理。失败发生在**已经存在的** `vibe route` 上。

设置：编码模型和路由模型都换成参数量 27B 的小模型（处理组的 `vibe route` 也是这个 27B）。观察：

| 观察项 | 结果 |
|---|---|
| 路由判断 | 2 个任务全部 no-match |
| 技能文件读取次数 | **0 次** |
| 技能内容是否进入上下文 | 从未进入 |
| 反而赢了的方案 | 静态脚手架（预先把结构固定注入） |

小样本、单次运行，并且有协议偏离。**不能**用来宣传「技能对弱模型有效」，也**不能**写成「所以我们发明了分层管线」。它只说明一件更窄的事：那一轮里，知识在文件里躺着，模型没去查。
...
**R7（量化驾驶舱）：题目写满。** 两组都能跑，栈接近。有技能组多了计划、测试、文档；成品打分评委分裂，不能写成一次结算胜利。记录：`.omx/artifacts/ab-quant-r7-report.md`。

**R8（量化平台）：题目交给另一个模型，不再铺路。** 有技能组先拒了一本不该用在从零盖房子上的治理说明书（这步是否决，不是失败），后面几乎都进了五步研发流程。项目管理说明书会逼你把能力做全，不会教你排版。于是台子功能很全，看起来像功能墙。**找错可能比没找更伤。** 预注册：`.omx/artifacts/ab-quant-r8-prereg.md`。

找错了，模型会认真执行一本错书。找错并拒绝，工程还能救；找错还严格执行，才会把仓库带进沟里。

## 8. 两条学习环，以及我们没有的那块

Warp 靠显式人类反馈：有人点踩、写下那句具体纠正，Improver 去改**已经存在的**技能。
...
反复出现的工具序列，达到 **≥5 次、成功率 ≥80%、且 ≥3 步** 后，可以变成 habit / instinct，**直接 boost 路由分数**，不必先变成 `SKILL.md`：

```mermaid
flowchart LR
    A["行为痕迹落盘"] --> B["≥5 次 且 成功率 ≥80% 且 ≥3 步"]
    B --> C["habit / instinct<br/>直接抬路由分数"]
```

**环 2：发现晋升（`vibe skill promote`）**

聚类出来的候选要另过一套门——**簇 size≥3 且 gold_rate≥60%，再经人审出草稿**——而且**不会自动变成技能文件注入**：

```mermaid
flowchart LR
    A["聚类簇"] --> B["size≥3 且 gold_rate≥60%"]
    B --> C["人审出草稿"]
    C --> D["才可能变成 SKILL.md"]
```

收窄后的 Memory 边界就这一句：**聚类候选不会自动变成 `SKILL.md` 注入。** habit / instinct 已经会改路由分数。

日常要让它少抽错，仍然是三件土办法，比再写一百本说明书更值：

- **点名。** 审查、TDD、调试，直接说出来或用斜杠命令。点名最高优先，不经过猜测。
...
| 入口 | 模型从 name + description 列表自选 | 四段级联；终端态是 no-match 或 fallback-llm |
...
- 实现层仍以 [routing-system.md](architecture/routing-system.md) 为准。
...
- 路由实现：[architecture/routing-system.md](architecture/routing-system.md)
- 上手：[QUICKSTART_USERS.md](QUICKSTART_USERS.md)
- 实验记录：仓库 `.omx/artifacts/ab-validation-wechat-deepdive.md`（R1–R6）、`ab-jet-weak-report-r6.md`（R6 协议偏离）、`ab-quant-r7-report.md`、`ab-quant-r8-prereg.md`（R8 目前只有预注册，结果见实验日志）
