...
>
> 这篇文章分两层。前半篇不需要任何背景：用一家后厨的比喻，讲清楚 AI 干活时身边的三样东西——记忆（Memory）、技能（Skill）、流水线（Harness）。后半篇是我们自己的实验记录：2026 年 8 月底到 9 月初，八轮双容器对照实验，五次平手，一次表面碾压，最后两轮把「题目的粗细」变成了主角。复盘的时候，这些结果把我们逼到了一个当初没想到的地方：
>
> **当一份需求写得足够清楚——边界条件、输入输出、每一步要什么——今天的 LLM 加上好 Harness，完全可以在没有技能的情况下完成工作。技能真正的岗位，不在 AI 干活的手上，而在人写需求的笔下。技能不是执行的外挂，是 spec 的补充形态。**
>
> 先把丑话说在前面：小样本、单次运行，不是统计证明；R7 有公开报告，R8 双臂已建成、盲评未结算——文中 R8 的判断是我们的人评，不是评分表。本文写的是我们怎么想、怎么测、被结果教育了什么，不是产品说明书。
...
旅程图外还有两轮，它们是排雷的收获，不是新的借口。R7（从真实行情做量化驾驶舱——题目写得很满，接口和数据几乎铺好）：两组都跑通了，隐藏 API 全绿，真实行情钉两边一致；评分表上评委第一次分裂——Grok 深评 16.5 对 17.0（裸臂反而高半分），Kimi 给 20 对 15（技能组明显更像可交付）。分裂本身就是答案：**写满的题，技能没有下手的地方。**R8 把题目反过来：换一个与出题人无关的模型（Kimi）只凭用户原话独立命题，任务书粗得多——只有模块清单，没有铺好的接口，没有钉死的数字。两臂都建完了，我们人肉翻代码、点面板的结论是技能组明显更完整（盲评未结算，这里记的是人评）。另外一笔污染值得记下——宿主 hook 曾把「只负责出题」的请求编成多意图执行计划：**差路由会在开考之前就把题目弄脏。**
...
| 好 Harness + 强模型 + 写满的题 + 方法论技能 | **冗余**（R1–R3、R7 连平） |
| 好 Harness + 强模型 + 粗的题 + 方法论技能 | **显形**（R8 人评：技能组明显更完整；盲评未结算） |
| 好 Harness + 弱模型 | **瓶颈在路由**——但 R6 里弱模型干到 22/25，接近强模型的 23.5 |
| 信息型技能（组织 runbook、冷门坑、项目私有约束） | **唯一还没测的格子** |

但排雷真正的收获不在这张表里。四个借口排完，剩下的那个解释，比「技能没用」有趣得多——它把那些平手拼成了一个形状。下一节讲这个形状。

## 5. 复盘：把八轮串起来的那根线

排完雷回头看，八轮数据各自为政：平手、平手、平手、否决、平手、翻面、平手、显形。复盘的时候，R6 里模型那句原话突然把所有轮次串成了一根线——
...
- **R8**：题目写粗——只剩模块清单——技能组第一次在产物上显形（人评）；
...
把 R6、R7、R8 三块拼图摆在一起，两个结论浮出来。

**结论一：技能可以提升模糊需求的表现。**R7 的题写满——接口铺好、验收钉死——两臂都跑通、隐藏 API 全绿；评分表上评委第一次分裂：Grok 深评 16.5 对 17.0，Kimi 给 20 对 15。分裂本身就是答案：差距小到两个评委能吵起来，方向都定不下来。R8 的题写粗——独立命题人只给模块清单，接口没铺、数字没钉——技能组把工作流从头走到尾，人评结论是它明显更完整。同一个 Harness、同一档模型，变的只有两样：题的粗细，技能的有无。**题越粗，技能越显形；题写满，技能归零。**这和「技能的价值在空白处」是同一句话，但现在两头都有了数据。

**结论二：spec 足够清晰，小模型可以做到大模型的能力。**R6 里 27B 弱模型在有支撑的环境（任务书 + 脚手架，全程零技能注入）交付 22/25，强模型同题 23.5/23.5——差的那 1.5 分在工程细节和收尾文档，核心的东西一样没少。弱模型自己的原话是「任务书本身就是完整的需求文档」——它觉得 spec 够清楚，就真的干到了接近强模型。施工图画满的时候，实习生和老师傅的差距，比所有人预想的小。

两个结论合起来，指向一条分工流水线：

```mermaid
flowchart LR
    A["复杂任务<br/>需求模糊"] --> B["最强模型编排<br/>拿技能图集拆解<br/>每个子任务：边界清楚、验收明确"]
    B --> C["子任务 1 · spec 写满"]
    B --> D["子任务 2 · spec 写满"]
    B --> E["子任务 N · spec 写满"]
    C --> F["小模型执行"]
...
**大模型当设计院，小模型当施工队。**复杂任务的编排和拆分交给最强模型：它拿着技能图集，把整件事拆成一个个边界清楚、验收明确的子任务——写出来的不是「做个驾驶舱」，而是「回测撮合走下一根开盘价 ±1 tick，印花税 5bp，隐藏 API 要 200」。这些写满的子任务书交给小模型执行——按 R6，弱模型在有支撑的环境里干到 22/25；按 R7，题写到那份上，执行时有没有技能已经无所谓了。人留在两个关口：拆完审图，交付验收。

丑话照说：这条流水线本身还没被当作一个整体检验过。八轮实验测的是它的三块零件——写满的题让技能冗余（R7）、粗的题让技能显形（R8）、有支撑的环境让弱模型接近强模型（R6）——不是整机。它现在是一个从数据里长出来的工程假设，不是定理。但它至少比另外两条默认路线——「什么活都上最贵的模型」和「什么活都塞技能」——多一条依据。
...
三条元经验，按价值排序：**预注册是给自己的护栏**（从 R5 起每一轮里最有用的单一制度）；**验证「处理真的发生了」和验证结果同样重要**（R6 的静默失败若没被抓住，结论方向完全相反）；**负结果要写下来发出去**（一连串平手逼出了 harness 假说、路由审计和重新定向，比任何一次「赢了」的 demo 产生的认知都多）。
...
下一步因此很清楚：弱模型格先测**路由质量**（让 27B 能正确命中、正确拒判），再谈注入；剩余唯一可检验的注入价值走**三臂设计**——裸 / 全量 dump / 智能路由，护城河只能是「路由 vs dump」的差值，且大概率要在 50+ 技能规模下才显形；信息型技能格（runbook、私有约束）单独立项——按第五节的框架，它是我们最看好的一格。此外按第五节的新推导，还欠一个整机实验：**大模型拿着技能拆解、小模型执行写满的子任务书**——三块零件都测过，整机没有。
...
- 不把「不够像」硬塞成命中。
- 不说聚类结果会自动变成技能文件。
- 不说「装了技能，成品一定更好」——强模型 + 好 Harness + 写满的题，连平；粗的题上技能显形，但那是人评，不是盲评。
- 不说没读到技能文件的对照证明了技能有效。
- 不把 R6 的完成度差距归因给技能内容。
- 不说徽章是上岗门——灯不是闸。
- 不说 Warp 缺了我们才能跑——他们已经在跑。
- 不把 R8 的人评判断写成盲评结算。
- 不说「大模型画图、小模型施工」的流水线已被整体检验过——八轮测的是它的零件，整机还是假设。
- 「技能是 spec 的泛化」是复盘得出的解释框架，与全部八轮观察相容，但**它本身还没有被一个专门设计的实验单独检验过**——它现在是一个假设，不是定理。
...
- R7 量化驾驶舱对照报告（含 Grok/Kimi 评分分裂）
  https://github.com/nehcuh/vibesop-py/blob/main/.omx/artifacts/ab-quant-r7-report.md
- R8 预注册（独立命题人、粗任务书、双臂协议）
  https://github.com/nehcuh/vibesop-py/blob/main/.omx/artifacts/ab-quant-r8-prereg.md
