...
> **成稿说明**：本章按 `docs/research-survey.md` 骨架 v0.1 §4 展开，仅展开骨架既有结论（H4/H5/H6、案例 4a-4e、F4/F5/F6/F10 对应面），不新增 F1-F10 之外的研究结论。
...
| 附录 B # | 数字 | 核对结果 | 出处 |
|---|---|---|---|
| #13 | evo C 191 文件 / 310 findings / ~1% | ✅ **已核对**：C.1 handback 表格 191（+73 skipped）/310/307 exact/0.0097；severity raw P0 3/P1 2/P2 6/NIT 299。⚠ 产物在 worktree `vibesop-evo-C-kimi`，主仓无 | `vibesop-evo-C-kimi/.omx/artifacts/evo-lane-C1-handback.md` |
...
| 数字 | 出处 | 本章核对状态 |
|---|---|---|
| evo A 稳定度 1.0/1.0（控制组/语义组，N=10×55） | `vibesop-evo-A-claude/.omx/artifacts/evo-lane-A-handback.md` §4 | **已核对（#9）**：两臂 primary/layer 稳定度均恰 1.0，0/55 不稳定 |
...
| near_miss 过灌 2/14 | `vibesop-evo-D2-claude/.omx/artifacts/evo-lane-D2-handback.md` | **已核对**：与 2/19 非矛盾（分子同、分母异） |
...
- [x] evo A handback artifact 路径 → `/Users/huchen/Projects/vibesop-evo-A-claude/.omx/artifacts/evo-lane-A-handback.md`（worktree 存在、文件在；主仓无同步副本，PR #120 合并后收口）
...
动工前对抗评审给出 **FIX BEFORE EXECUTE 总判定、8 个 CONFIRMED FLAW**（`docs/decisions/_review-instinct-loop-brief.md`）。最要命的三条都在并发与状态管理上：
...
| 22 | 1573 spans；task_id 0% / role_id 0% / parent_span_id 5.8% / orchestration_id 不存在 / mirror parent_session 空 | `docs/decisions/_review-dashboard-v3-brief.md` 增量 2 | 已核对 |
...
- 论据：§5 H7；`vibesop-evo-A-claude/.omx/artifacts/evo-lane-A-handback.md`
...
| III | evo A profile-semantic | N=10×55，控制组 | 稳定度 1.0/1.0；embedding 180/550 | 证伪(b) | evo-A worktree handback ⚠路径 |
...
| 9 | evo A 稳定度 1.0/1.0（N=10×55，0/55 不稳定） | `vibesop-evo-A-claude/.omx/artifacts/evo-lane-A-handback.md` | 09-11 | ✅ 已核对 |
