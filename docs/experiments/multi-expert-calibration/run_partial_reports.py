...
def run(task_id,arm,root):
    ...
    def call(stage,request,cap):
        ...
        record={'stage':stage,'messages':messages,'max_tokens':cap,'model':MODEL,'temperature':0,'thinking':{'type':'disabled'},'started_at':datetime.now(timezone.utc).isoformat()}
        n=len(calls);calls.append(record);save(d/f'call-{n:02d}-request.json',record)
        try:
            response=client.chat.completions.create(model=MODEL,messages=messages,temperature=0,max_tokens=cap,extra_body={'thinking':{'type':'disabled'}})
            ...
            if finish!='stop' and (finish!='length' or stage in ('deliver','check')):
                raise RuntimeError('incomplete_response:'+str(finish))
            if finish=='length':record['partial_internal_stage']=True
    ...
    result['partial_internal_stages']=[c['stage'] for c in calls if c.get('partial_internal_stage')]
...
if __name__=='__main__':
    ...
    prior=ROOT/'runs/20260913T010758Z'
    prior_manifest=json.loads((prior/'manifest.json').read_text())
    assert hashlib.sha256((ROOT/'tasks.py').read_bytes()).hexdigest()==prior_manifest['hashes']['tasks.py']
    check=json.loads((prior/'self-check.json').read_text())
    check['reused_from']=str(prior/'self-check.json')
