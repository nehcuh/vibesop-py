...
| 要找什么 | 入口 |
|---|---|
| 当前项目定位、源码与公开发行边界 | [项目定位](POSITIONING.md)、[项目状态](PROJECT_STATUS.md) |
| 当前产品使用与开发文档 | 本页后续各节 |
| 研究结论、研究线与稳定证据 | [研究索引](research/README.md) |
...
- **[给 AI 找对说明书](skill-routing-explained.md)** — 技能路由科普（对照实验发现、和同类差别、怎么用和进化）。技能主题的实践解读；项目总定位以 [POSITIONING.md](POSITIONING.md) 为准。
- **[研究综述：把「可信」从声称变成可测量](research/research-survey.md)** — 2026-04→09 全部实验与研究总账（四条实验线、F1-F10 综合发现、26 条外部对照、三层建议；数字逐项核对 + grok 逐章复审）。**研究总账以这篇为准。**
- **[公众号长文：给 AI 配了七个专家，为什么未必比一个模型强？](essays/agent-trust/article.md)** — 2026-09-14 图文更新：技能、spec、强弱模型、专家委员会、反复评审与记忆；含9张图及[手机阅读预览](essays/agent-trust/preview.html)。
