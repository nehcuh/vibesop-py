...
description: >-
  Comprehensive codebase audit across 28 quality dimensions (architecture, security,
  stability, performance, testing, maintainability, design, release, documentation,
  observability, configuration, data integrity, privacy, accessibility, supply chain,
  cost, AI/LLM safety, frontend state, backend API, dependency weight, code consistency,
  comment coverage, concurrency). Supports full, incremental, and focused audits
  with HTML/JSON/Markdown report output.
intent: 全维度代码库审计 — 支持 full/incremental/专项模式，输出 HTML/JSON/MD 报告
trigger_when: >
  用户要求全面代码审计、仓库健康报告、PR review diff 审计，
  或提及 audit、审计、代码质量检查、代码审查 等
triggers:
  - "audit"
  - "审计"
  - "code audit"
