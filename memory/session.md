...
- 交付物：`docs/research/research-survey.md` 终稿 v1.0（~1700 行、21 项实验、F1-F10、E1-E26 外部对照、11 图全渲染、9 轮 grok 评审全闭环）。
...
- 用户核心洞察（写入第 5 节）：需求/spec 写满时 LLM+Harness 无需技能；skill = 需求设计的泛化、spec 的补充（spec 一次一图，skill 是图集；老师傅手感=没画出来的施工图）；R6 模型原话「任务书即完整需求」为引线；R5「评分表打平 ≠ 意图达成」补强 grill 论证
- grill-me（mattpocock/skills，外链已核实）作为社区印证：spec 缺口探测器，事实它查/决策你拍，disable-model-invocation 只能人点名
- 结构：后厨类比（memory 便签/skill 菜谱卡/harness 焊死的规矩/第 4 角色=点菜人 spec）→ Warp 循环 → 三层问题 → R1-R6 叙事 → 三道防线图（spec/模型先验/harness 盖需求空白，剩余缺口=技能地盘）→ 价值版图迁移（执行层冗余→设计层+信息层）→ 路由/方法论账单/不声称清单（新增：泛化框架本身是假设未单独检验）
- 7 张 mermaid 图，全部信息取自 R1-R6 报告原文（80 次读取换不来 1 分、2/2 no-match、5/7 误注入恒 82% 等）
- 用户复审反馈：中段实验叙事太干易划走 → §3 改「考试/封卷/验药」语感，§4 重写为「借口排雷记」连续剧结构（三条选题直觉→每轮先讲怎么选题为什么该赢→结局→借口→下轮冲借口去），新增旅程图（R1→R6 借口链），监考老师塞小抄框架路由审计，香槟梗 R6 翻面
- Next: R8 结算后回写 §9 假设条目（「技能是 spec 的泛化」待专门实验检验）；该文即公众号素材
- Recorded: yes — 写作模式入 project-knowledge Reusable Patterns；已提交（docs/article）

### S67 (2026-09-03) [vibesop-py] v8.2.0 对抗评审 REQUEST CHANGES → 定点修 HIGH

- adversarial-review `7f2de48..967e134`: finders 5/5, 27→23 确认 / 4 驳回 / 0 未核实, **REQUEST CHANGES** (HIGH 13). 报告 `.omx/artifacts/adversarial-review-7f2de48-967e134.md`
- 合适流程：kimi-gated-fix 形态（已定位 file:line），主会话控编辑，不跑全仓深诊。未启动 `fix-from-review` 落地（文件重叠）。
- 已修确认 HIGH 簇：demote `mode=no_match`；payload `primary=None`；skill_file_note；decomposer `filter_routable`；SKILL.md+拒 symlink；CLI runtime_scan；source_lookup 穿 TTY/executor；doctor hard-red e2e；CHANGELOG confirmation_mode 补录口径；grok `Generated by VibeSOP vX`
- [executed] 定向 394 passed / 3 skipped；ruff 绿
- Claude `-p`：带 Read 工具挂死；`--tools ""` 编造不存在的 SkillInjection API 后 APPROVE。产物标不可靠：`.omx/artifacts/ask-claude-fail-closed-residual-20260903.md`
- 未提交
- Recorded: no

### S66 (2026-09-03) [vibesop-py] 按对抗评审结果定点修复 + Claude 复审

- Routing: multi-intent deep-diagnosis + code-review + slash-orchestrate. Override: Phase 1 全仓深诊会丢掉已冻结的 5 路评审；code-review 不是 Claude CLI 复审。合适流程 = kimi-gated-fix 形态（已定位 file:line）+ `$ask claude`。
- adversarial-review 仍在 Verify（finders 5/5，27 条，concurrency 空）。未改 src/。
- 独立抽查已站住：demote 不改 mode；TTY 仍绿 Matched；annotate 对象路径无 skill_file_note / 漏传 lookup；decomposer 未 filter_routable；`_if_file` 无 PathSafety；CHANGELOG confirmation_mode 非本 pull。
- 产物：`.omx/artifacts/pull-20260903-fix-plan.md`，workflow `.grok/workflows/fix-from-review.rhai`
- Next: Verify 出确认票 → 只修 confirmed HIGH → pytest → `claude -p`
- Recorded: no

### S65 (2026-09-03) [vibesop-py] pull latest + 多路独立对抗评审

- Routing selected builtin/code-review (88%); override: user named 多路独立对抗评审, matching registered workflow `adversarial-review` (5 finders + refute-first verifiers). code-review is pre-push walkthrough of local pending changes, not this protocol.
- Pull: `7f2de48` (v8.1.4) → `967e134` (v8.2.0, tag v8.2.0); ff-only. 8 commits, 79 files +1828/−349.
- Frozen: `.omx/artifacts/review-diff-7f2de48-967e134.patch`
- Instructions: `.omx/artifacts/pull-20260903-review-instructions.md`
- Theme: fail-closed source_file/demote/skill_file + doctor freshness + CLI lookup + CHANGELOG 8.2.0
...
- Kimi REQUEST CHANGES（P1）：`vibe route --slash "/session-end"` 非 `/vibe-*`，CLI Exit(1)。
- Claude APPROVE（冻结 95ed6d2）；P2 不阻塞。
- [executed] `--slash "/session-end"` Exit(1)；`vibe route "session-end"` fallback-llm（短查询+守卫技能），`skill_file` 空；`vibe skills info builtin/session-end` 打印 `core/skills/session-end/SKILL.md`。
- 回退文案改为 skills info。定向 36 passed。
- Recorded: no
...
- 双路：Kimi REQUEST CHANGES（P1 session-end --slash）/ Claude APPROVE → 吸收见 S62。
- Recorded: no
...
- 独立镜头 COMMENT：candidate `source_file` 未穿进 inject。已补：SkillRoute.metadata + CandidateManager.source_file_for + handle_query 传入 inject。
- [executed] 搜索根外 orphan 经 candidate source_file 注入成功。
- Recorded: no
