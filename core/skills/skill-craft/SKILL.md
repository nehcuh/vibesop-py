...
- TODO: record the success and failure of the source executions this skill was distilled from（来源成败：来源执行的成功与失败各是什么）
...
> **晋升四要素（F9）**：`## Prerequisites` / `## Counterexamples` /
> `## Verification` / `## Source Outcomes` 是与 `vibe skill promote`
> 草稿共用的同一份 canonical 结构（人审必填：前提、反例、验证方法、
> 来源成败）。promote verifier 对仍为 TODO 占位的要素报
> `promotion-element-missing: <element>` WARN 码——只报不拦，永不 FAIL、
> 永不阻断 activate（gate34：过滤自动化、不过滤人审）。改名必须两侧同步。
