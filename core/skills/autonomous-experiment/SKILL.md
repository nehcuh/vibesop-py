...
3. Create the experiment worktree and scaffolding (no CLI — direct git + files):
   ```bash
   git worktree add .experiment/worktree -b experiment/<name>
   ```
   Then create `.experiment/results.tsv` (header: `iteration\tbelief\tpredicted\tactual\tcompound\tverdict`) and `.experiment/beliefs.md` at the project root — experiment metadata stays OUTSIDE the worktree so it is never merged back.
4. Establish baseline: run evaluator on **unmodified** code. Record baseline score in results.tsv.

### Phase 2: Experiment Loop

ALL file modifications, commits, and resets in this loop happen INSIDE the worktree (`.experiment/worktree`, checked out on `experiment/<name>`) — either `cd` there first or prefix every git command with `git -C .experiment/worktree`. Metadata files (`.experiment/beliefs.md`, `.experiment/results.tsv`) stay at the project root, outside the worktree.

LOOP (until `constraints.max_iterations` reached OR no improvement for `constraints.stale_threshold` consecutive iterations):

1. **Read `.experiment/beliefs.md`**. Select a belief to test or combine beliefs for a new hypothesis.
2. **Write prediction** to `.experiment/beliefs.md` — predicted scores per rubric dimension + reasoning.
3. **Modify files** in `scope.modifiable` only (inside the worktree). NEVER touch `scope.readonly` files.
4. **Commit** in the worktree: `git -C .experiment/worktree add -A && git -C .experiment/worktree commit -m "<iteration description>"` (`-am` alone misses untracked new files).
5. **Run evaluator** (against the worktree — running it at the project root would score unmodified code) based on `evaluator.type`:
...
6. **Compare** actual vs predicted scores. Write results to `.experiment/beliefs.md`.
7. **Attribution**: Which belief was right? Which was wrong? Update beliefs accordingly.
8. **Record** to `.experiment/results.tsv` by appending the row directly.
9. **Keep or Discard**:
   - Compound score improved → `keep` (advance branch).
   - Compound score worsened or equal → `discard`: `git -C .experiment/worktree reset --hard HEAD~1`.
...
4. Output merge instruction: from the main checkout, `git merge experiment/<name>` to apply the best result (after human review).
...
- **Max 20 beliefs** in `.experiment/beliefs.md`. When full, rewrite (not append) to keep most relevant.
- The human will review `.experiment/results.tsv` and summary when they return.
