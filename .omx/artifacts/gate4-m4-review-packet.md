# 门禁4 复审包:M4 backlog(缓存统一 + 配置可见性 + nit 包)

## M4 变更说明
1. M4a(triage_service.py):删除 CacheManager 磁盘缓存路径(_get_cache/_set_cache),triage 跨进程统一走 TriageCache;last-good 置信度 ×0.7 衰减并记录原值;熔断/预算 early-return 前先尝试 last-good。
2. M4b(cli/main.py, llm_config.py, unified.py):api_key 空时 config 被忽略的 warning;get_llm_config 回退全局配置/解析失败的日志;prompt_builder 缺失 warning;triage 胜出时补 scenario 参与计数。
3. M4c(triage_recall.py, build_eval_from_logs.py):embedding 缓存单锁 RMW + 已删 skill 条目裁剪;merge_confirmed 空主集/[] 时整体重写。

注意:diff 相对 HEAD 包含 M1-M3 已审内容,请聚焦上述 M4 增量。

## diff: triage_service.py
diff --git a/src/vibesop/core/routing/triage_service.py b/src/vibesop/core/routing/triage_service.py
index 8a2a8d5..edc5859 100644
...
@@ -35,15 +39,46 @@ class TriageService:
...
@@ -77,24 +112,6 @@ class TriageService:
...
@@ -115,10 +132,65 @@ class TriageService:
...
+        # The candidates_hash check inside lookup covers the B1 aliveness
+        # semantics: a changed candidate set demotes the entry to stale, and
...
@@ -135,11 +207,7 @@ class TriageService:
...
@@ -211,14 +279,23 @@ class TriageService:
...
@@ -228,23 +305,36 @@ class TriageService:
...
@@ -381,21 +471,90 @@ class TriageService:
...
+        current candidate set (a removed skill is never resurrected).
+
+        The recorded confidence is decayed (×0.7) so a stale result never
+        competes with a fresh LLM result at full weight; the original value
+        is kept in metadata as ``last_good_original_confidence``. The decayed
...
## diff: llm_config.py
diff --git a/src/vibesop/core/llm_config.py b/src/vibesop/core/llm_config.py
index 9a2d438..b94f3b3 100644
--- a/src/vibesop/core/llm_config.py
+++ b/src/vibesop/core/llm_config.py
...
## diff: cli/main.py(_build_llm_factory 区域)
diff --git a/src/vibesop/cli/main.py b/src/vibesop/cli/main.py
index 703ce65..2936976 100644
...
@@ -157,6 +189,12 @@ class UnifiedRouter(
...
@@ -408,6 +446,14 @@ class UnifiedRouter(
...
@@ -543,7 +589,17 @@ class UnifiedRouter(
...
+        elif early_match is not None:
             ...
             return self._build_match_result(
                 query,
@@ -558,19 +614,31 @@ class UnifiedRouter(
                 context,
             )
...
+                self._record_layer(RoutingLayer.SCENARIO)
             return self._build_match_result(
                 query,
                 match,
@@ -584,6 +652,29 @@ class UnifiedRouter(
                 context,
             )
...
@@ -624,9 +715,9 @@ class UnifiedRouter(
...
@@ -638,9 +729,9 @@ class UnifiedRouter(
         else:
             # Index standalone
...
@@ -830,6 +921,13 @@ class UnifiedRouter(
         Returns:
...
## build_eval_from_logs.py merge_confirmed 段
