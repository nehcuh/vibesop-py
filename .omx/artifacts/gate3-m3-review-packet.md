# 门禁3 复审包:M3(embedding 召回 + scenario 权威废除 + CJK bigram + 垃圾过滤)

## 变更文件清单
 M src/vibesop/core/analytics.py
 M src/vibesop/core/config/manager.py
 M src/vibesop/core/routing/_layers.py
 M src/vibesop/core/routing/triage_service.py
 M src/vibesop/core/routing/unified.py
 M tests/core/routing/test_index_layer.py
 M tests/core/test_analytics.py
 M tests/unit/core/routing/test_triage_service.py
?? .vibe/triage_cache.json
?? .vibe/triage_cache.lock
?? scripts/build_eval_from_logs.py
?? scripts/replay_routing.py
?? src/vibesop/core/routing/triage_cache.py
?? src/vibesop/core/routing/triage_recall.py
?? tests/benchmark/routing_eval_extended.yaml
?? tests/core/routing/test_scenario_demotion.py
?? tests/scripts/
?? tests/unit/core/routing/test_triage_cache.py
?? tests/unit/core/routing/test_triage_recall.py
?? tests/unit/test_build_eval_from_logs.py

## git diff: unified.py
diff --git a/src/vibesop/core/routing/unified.py b/src/vibesop/core/routing/unified.py
index 08417a5..eee73d8 100644
...
@@ -543,7 +552,17 @@ class UnifiedRouter(
...
+        elif early_match is not None:
             ...
             return self._build_match_result(
                 query,
@@ -584,6 +603,30 @@ class UnifiedRouter(
                 context,
             )
 
+        # Scenario fallback: triage was unavailable, bypassed, or returned
+        # nothing usable, so the demoted scenario match becomes the result.
+        # Interaction with the triage short-query bypass: for short keyword
+        # queries (< ai_triage_short_query_bypass_chars) triage is bypassed
+        # by design, so a scenario hit almost always lands here — that is
+        # intended; the demotion only matters for long queries where triage
+        # actually runs. The fallback is flagged so downstream consumers can
+        # tell "scenario won by default" from a triage-arbitrated match.
...
@@ -624,9 +667,9 @@ class UnifiedRouter(
...
@@ -638,9 +681,9 @@ class UnifiedRouter(
         else:
             # Index standalone
...
@@ -830,6 +873,26 @@ class UnifiedRouter(
         Returns:
...
## git diff: triage_service.py(M2 已审,本轮只看 embedding recall 接入——prefilter 与 __init__)
+        embedding_recall: EmbeddingRecall | None = None,
     ) -> None:
         self._config = config
         self._cost_tracker = cost_tracker
@@ -44,6 +50,31 @@ class TriageService:
...
@@ -77,24 +108,6 @@ class TriageService:
...
@@ -117,8 +130,63 @@ class TriageService:
...
@@ -135,11 +203,7 @@ class TriageService:
...
@@ -211,14 +275,24 @@ class TriageService:
...
@@ -228,23 +302,36 @@ class TriageService:
...
@@ -399,3 +486,80 @@ class TriageService:
...
the caller falls back to the KeywordMatcher path.
"""
...
## Replay 验证数据(--no-llm,project_root=cmspark 自动推导)

- 门禁1 基线(旧代码,错误的 vibesop-py root):一致率 11.0%,fallback 51.5%,scenario 421
- M3 后(正确的 cmspark root):一致率 19.1%,fallback 44.2%(817/1850),scenario 0,semantic_index 73,keyword 540,levenshtein 407
- 关键解读:两次口径不同(root 修正)不能直接相减;scenario=0 说明 cmspark 无 scenario 配置,门禁1 的 421 次 scenario 命中是用错 skill 注册表的假象;scenario 降级对本语料无影响;embedding 召回只在 LLM 路径生效,--no-llm replay 无法验证其收益。
