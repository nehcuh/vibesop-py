# 门禁2 复审包:M2 持久化 triage 缓存 + M1 收尾修复

## 说明
门禁1 已通过(PASS_WITH_NITS)。本包重点审:(A) M2 新增——triage_cache.py + triage_service.py/manager.py 改动;(B) M1 收尾——build_eval_from_logs.py redact join/原子写、replay_routing.py --project-root、analytics.py 删 query_hash/负秒 clamp。M1 核心(mislabel 修复、隐式信号主体)门禁1 已审,不必重复。

## git diff(已跟踪文件)
diff --git a/src/vibesop/core/analytics.py b/src/vibesop/core/analytics.py
index 6c14c7c..fc70731 100644
...
@@ -66,6 +71,109 @@ class ExecutionRecord:
         )
...
+    file (same pattern as ``.vibe/instincts.jsonl.lock``).
+
...
@@ -77,10 +185,20 @@ class AnalyticsStore:
...
@@ -120,6 +136,31 @@ class TriageService:
         if cached:
...
@@ -135,11 +176,7 @@ class TriageService:
...
@@ -214,11 +251,20 @@ class TriageService:
                         },
                     )
...
@@ -399,3 +445,76 @@ class TriageService:
...
+                candidate = next(
+                    (c for c in candidates if c["id"].lower() == skill_id.lower()), None
+                )
+            if candidate is None:
+                return None
+            return SkillRoute(
...
@@ -163,3 +165,157 @@ class TestAnalyticsStore:
...
## 新文件: src/vibesop/core/routing/triage_cache.py
