# 门禁4b(终)复审包:gate 前移 + nit

## 修复内容
1. budget/circuit gate 移到 prefilter 之前(gate 关闭且缓存 miss 时不再白付 embedding recall);gate 拒绝路径的 last-good 尝试随 gate 移动。
2. _last_good_route metadata candidates_sent=0(未发 LLM,与 fresh-hit 口径一致)。
3. models.py SkillRoute.to_dict 补 description(纯新增,向后兼容;此前持久化缓存条目 description 恒空)。

## 最终顺序
enable/configured → augmented_query → lookup(全量) → fresh 命中返回 → budget gate → circuit gate → prefilter → LLM → store(全量)

## diff: triage_service.py
diff --git a/src/vibesop/core/routing/triage_service.py b/src/vibesop/core/routing/triage_service.py
index 8a2a8d5..b906c47 100644
...
@@ -75,9 +114,64 @@ class TriageService:
...
@@ -86,40 +180,36 @@ class TriageService:
...
@@ -135,11 +225,7 @@ class TriageService:
...
@@ -211,14 +297,23 @@ class TriageService:
...
@@ -228,23 +323,36 @@ class TriageService:
...
@@ -381,21 +489,94 @@ class TriageService:
...
## diff: models.py
diff --git a/src/vibesop/core/models.py b/src/vibesop/core/models.py
index 5f1d689..fb75219 100644
--- a/src/vibesop/core/models.py
+++ b/src/vibesop/core/models.py
