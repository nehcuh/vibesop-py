...
- **存量部署与声明面修复（pull-20260903 三路评审批C）**: wheel 升级只换
  ...
  按 5 个可构建平台（claude-code/grok-build/kimi-cli/opencode/pi）实际渲染
  产物清单扫描 always-loaded 文件与中心库 `slash-*/SKILL.md`，发现
  「命令 agent 猜 `skills/<id>/SKILL.md`」（含 `<matched-skill>` 占位句式
  与 session-end 具体死路径）或版本标记落后即报出并给出重部署命令
  （`vibe build <platform> --output <配置目录>` /
  `vibe skills sync <platform> --force`）；段落级判定不误伤当前模板的
  「do not guess」禁令句与 `.pi/skills/` 生成树备注。claude-code/kimi-cli/
  opencode/pi 四家均扫 `docs/session-lifecycle.md`（session-end 死路径的
  主载体）。cursor 暂不覆盖：`vibe build cursor` 渲染产物为零
  （ConfigRenderer 拒绝该平台，构建支持另行修复）。版本标记量纲修复：
  部署文件的 `Generated by VibeSOP vX` 此前渲染 manifest 常量版本
  （恒 1.0.0，重部署也消不掉 staleness 告警）——模板标记改绑包版本
  （新增 `vibesop_version` 上下文键；kimi-cli slim AGENTS.md 去掉
  manifest 版本覆盖、空 skills manifest 不再渲染 Skills 引用段（对齐
  file_based 行为）；claude-code `vibesop-track.sh` hook 渲染去掉硬编码
  `5.2.0`；pi `install_hooks`/`_render_extension` 走包版本而非占位
  `0.0.0`——此前会渲染出无法比对的空版本标记），fresh build 实测 5 平台
  标记均为包版本。opencode 补扫 `docs/routing.md`；session-end 残留
  正则加左边界（`vibe skills info` 打印的真实 Source 路径如
  `core/skills/session-end/SKILL.md` 不误报）。doctor exit 码语义拍板：
  顺带修复汇总 bug（`all_ok` 此前解包到检查名恒真，doctor 从未非零
  退出），同时把 Platform Integrations / Hook Status / Deployment
  Freshness 定为 advisory ⚠️——只告警不参与 exit 码聚合
  （`_doctor_exit_ok` 纯函数），doctor 保持诊断定位，环境漂移（未装
  agent CLI / 待重部署）不再 hard-fail 脚本；hard-red（Python/依赖/
  配置/LLM/Skill Health）才以非零码退出。Pi 全链路去猜路径：
  `AGENTS.md(.project).j2` 路由协议句、`prompts/vibe-route.md(.j2)`、
  `docs/session-lifecycle.md(.j2)`、`vibesop-track.ts(.j2)` 改为跟
  `skill_file` / `NEXT STEP` / `vibe skills info builtin/session-end`
  （打印真实 Source file），`docs/skills.md(.j2)` 去掉
  `read .pi/skills/<skill-id>/SKILL.md` 生成树读命令（该树可能为空），
  `.pi/skills/` 仅作生成树副本位置备注，签入副本同步刷新。
  执行清单与 hook 注入的长度口径分叉文档化：
  `StepManifest.skill_content` 有意不套 `SkillInjector.MAX_INJECT_LENGTH`
...
- **匹配必须可注入 SKILL.md（fail-closed）**: 发现层 rglob 能看见
  ...
  > 2026-09-03 事后更正（pull-20260903 评审）：本条「候选缺少 `source_file`
  > 不可路由」当时仅对「设置了但文件缺失/不可读」的候选成立——
  > `source_file=None`/缺键的候选在 8.1.4 仍可路由，硬丢弃在后续批B 落地；
  > 「路由命中把 `source_file` 写入 SkillRoute.metadata」当时也只在部分层
  > 成立（token-overlap 与 AI triage 主导层缺穿），全链路穿透同样在批B。
  > 另：升级 wheel 不会刷新已部署的 always-loaded 文件（旧部署仍命令猜
  > `skills/<id>/SKILL.md`，与注入的真路径打架），升级后需重跑
  > `vibe build <platform> --output <配置目录>`；`vibe doctor` 的
  > Deployment Freshness 检查可定位残留。
