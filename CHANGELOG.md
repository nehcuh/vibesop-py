...
- **存量部署与声明面修复（pull-20260903 三路评审批C）**: wheel 升级只换
  ...
  manifest 版本覆盖；claude-code `vibesop-track.sh` hook 渲染去掉硬编码
  `5.2.0`），fresh build 实测 5 平台标记均为包版本。顺带修复
  doctor 自身汇总 bug：`all_ok` 此前解包到检查名（字符串恒真），doctor
  从未以非零码退出——现按真实状态聚合，环境问题不再静默绿（本机实测
  Skill Health 6 critical + 部署残留会真实红）。Pi 全链路去猜路径：
  ...
  （打印真实 Source file），`.pi/skills/` 仅作生成树副本位置备注，
  签入副本同步刷新。执行清单与 hook 注入的长度口径分叉文档化：
