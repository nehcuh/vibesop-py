...
class TestGeneratedCopyDoesNotGuessFlatSkillPath:
    ...
    def test_pi_extension_markers_bind_package_version(self) -> None:
        """install_hooks renders extensions without manifest context
        (`_render_extension`); a placeholder version would emit an
        unparseable marker that `vibe doctor` can neither flag as stale
        nor confirm as current."""
        ...
        for name in ("vibesop-route.ts.j2", "vibesop-track.ts.j2"):
            text = adapter._render_extension(name)
            assert f"Generated by VibeSOP v{__version__}" in text, name
