...
class TestComputeAmbiguity:
    ...
    def test_fully_clear(self):
        ...
        result = compute_ambiguity(intent=1.0, outcome=1.0, scope=1.0, constraints=1.0, success=1.0)
        assert result == pytest.approx(0.0)

    def test_fully_ambiguous(self):
        """All dimensions at 0.0 should yield 1.0 ambiguity."""
        result = compute_ambiguity(intent=0.0, outcome=0.0, scope=0.0, constraints=0.0, success=0.0)
        assert result == pytest.approx(1.0)

    def test_neutral(self):
        """All dimensions at 0.5 should yield 0.5 ambiguity."""
        result = compute_ambiguity(intent=0.5, outcome=0.5, scope=0.5, constraints=0.5, success=0.5)
        assert result == pytest.approx(0.5)

    def test_weighted_intent_dominant(self):
        """Intent has the highest weight (0.30)."""
        result = compute_ambiguity(intent=0.0, outcome=1.0, scope=1.0, constraints=1.0, success=1.0)
    ...
    def test_weighted_success_least(self):
        ...
        result = compute_ambiguity(intent=1.0, outcome=1.0, scope=1.0, constraints=1.0, success=0.0)
    ...
    def test_clamped_to_0(self):
        ...
        result = compute_ambiguity(intent=2.0, outcome=2.0, scope=2.0, constraints=2.0, success=2.0)
        assert result == pytest.approx(0.0)
