...
class TestLazyEmbeddingMatcher:
    ...
    def test_match_forwards_default_top_k(self):
        """Omitting top_k must forward IMatcher/EmbeddingMatcher default 10."""
        cfg = MatcherConfig()
        lazy = LazyEmbeddingMatcher(cfg)
        mock_real = MagicMock()
        mock_real.match.return_value = []
        lazy._real = mock_real
        candidates = _make_candidates("test")
        lazy.match("query", candidates)
        mock_real.match.assert_called_once_with("query", candidates, None, top_k=10)
