...
def test_minimal_cli_payload_is_blocked_and_keeps_verifier(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    attach_skill_file_payload(payload, routed)
    # 8.3.1 (G-1): the formatter itself demotes blocked plans to no_match —
    # the minimal shape no longer carries steps for blocked plans.
...
def test_minimal_cli_ready_plan_keeps_authoritative_paths(tmp_path, monkeypatch):
    ...
    plan = make_plan(tmp_path)
    # 8.3.1 (G-1): the formatter gates on execution_ready; a ready plan must
    # be annotated first (production: the orchestrator annotates before any
    # consumer reaches the formatter).
