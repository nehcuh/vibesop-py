...
class TestReplayBasic:
    def test_replay_single_trace(self, runner: CliRunner, tmp_path: Path) -> None:
        span_file = tmp_path / "spans.jsonl"
        _write_spans(
            span_file,
            [
                _make_span(
                    "task-1", "trace-1", kind="task", name="route:hello", skill_id="mcp-install"
                ),
                _make_span(
                    "llm-1",
                    "trace-1",
    ...
    def test_replay_no_span_file(self, runner: CliRunner, tmp_path: Path) -> None:
    ...
    def test_replay_empty_span_file(self, runner: CliRunner, tmp_path: Path) -> None:
...
class TestReplayFiltering:
    def test_trace_id_prefix_filter(self, runner: CliRunner, tmp_path: Path) -> None:
        span_file = tmp_path / "spans.jsonl"
        _write_spans(
            span_file,
            [
    ...
    def test_trace_id_no_match_exits_1(self, runner: CliRunner, tmp_path: Path) -> None:
        span_file = tmp_path / "spans.jsonl"
        _write_spans(span_file, [_make_span("t1", "trace-aaa", kind="task")])

        result = runner.invoke(app, ["replay", "--span-file", str(span_file), "--trace-id", "zzz"])
        assert result.exit_code == 1

    def test_limit_caps_traces(self, runner: CliRunner, tmp_path: Path) -> None:
        span_file = tmp_path / "spans.jsonl"
        spans = []
        for i in range(5):
            spans.append(
                _make_span(
                    ...
                    kind="task",
                    name=f"route:q{i}",
                    started_at=f"2026-07-22T10:0{i}:00+00:00",
                )
            )
        _write_spans(span_file, spans)

        result = runner.invoke(app, ["replay", "--span-file", str(span_file), "--limit", "2"])
        assert result.exit_code == 0
        assert "2 trace(s)" in result.output


class TestReplayJsonOutput:
    def test_json_output_structure(self, runner: CliRunner, tmp_path: Path) -> None:
        span_file = tmp_path / "spans.jsonl"
        _write_spans(
            span_file,
            [
        ...
        result = runner.invoke(app, ["replay", "--span-file", str(span_file), "--json"])
        assert result.exit_code == 0
...
class TestReplayTree:
    def test_nested_tree_indentation(self, runner: CliRunner, tmp_path: Path) -> None:
    ...
    def test_mid_tree_orphan_marked(self, runner: CliRunner, tmp_path: Path) -> None:
    ...
    def test_zero_duration_rendered_not_dash(self, runner: CliRunner, tmp_path: Path) -> None:
    ...
    def test_error_status_spans_render(self, runner: CliRunner, tmp_path: Path) -> None:
...
class TestMetricsCommand:
    ...
    def test_metrics_no_data_graceful(self, runner: CliRunner, tmp_path: Path) -> None:
        span_file = tmp_path / "spans.jsonl"
        _write_spans(
            span_file,
            [
    ...
    def test_metrics_returns_aggregated_values(self, runner: CliRunner, tmp_path: Path) -> None:
        span_file = tmp_path / "spans.jsonl"
        _write_spans(
            span_file,
            [
    ...
    def test_metrics_project_id_filter(self, runner: CliRunner, tmp_path: Path) -> None:
    ...
    def test_metrics_excludes_spans_outside_window(self, runner: CliRunner, tmp_path: Path) -> None:
