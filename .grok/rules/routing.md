...
**Before every non-trivial task, call `vibe route` to look up whether a skill applies.**
...
1. Follow the injected skill body. If a `NEXT STEP` / `skill_file` path is present, read that file — do not guess `skills/<id>/SKILL.md`
...
- **No match**: a successful outcome (`Proceeding in normal mode`); do not invent a skill
